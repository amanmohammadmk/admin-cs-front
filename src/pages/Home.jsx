...
function Home() {
  ...
  useEffect(() => {
    const token = document.cookie.includes('token') ? document.cookie.split('; ').find(row => row.startsWith('token')).split('=')[1] : null;

    if (!token) {
      window.location.href = '/'; // Redirect to the home page if token doesn't exist
      return; // No need to proceed if token doesn't exist
    }
    ...
    axios.post('http://localhost:3001/home')
      .then(response => {
        ...
        if (response.data.success) {
          setAuthenticated(true); // Set authenticated state to true if authentication is successful
        } else {
      ...
      .finally(() => {
        setLoading(false); // Set loading state to false after authentication check completes
      });
  }, []);

  const handleLogout = () => {
    document.cookie = 'token=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;'; // Delete token from cookies
